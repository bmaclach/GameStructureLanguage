...
preCompile = updateIds . updateComps . applyModifiers
...
applyModifiers (G p rs wc tbs) = 
    G p (remove0Rounds $ concatMap (applyRoundModifiers 0 . return) rs) wc tbs
...
applyModifier (R pl n ((Jst rn tr pn p):ml)) m = [R pl (rn - m - 1) ml, 
  R (addPhaseToPhaseList pl p tr pn) 1 ml, R pl (n - (rn - m)) ml]
applyModifier (R pl n ((From rn tr pn p):ml)) m = [R pl (rn - m - 1) ml, 
  R (addPhaseToPhaseList pl p tr pn) (n - (rn - m) + 1) ml]
...
updatePhaseComps (p@(Act (Comp (Scored _ _))):pl) n ci = 
  p : updatePhaseComps pl (n+1) ci
updatePhaseComps ((Act (Comp c@(Placed _ il _ _))):pl) n ci = 
  Act (Comp (updateComp c n ci)) : updatePhaseComps pl (n+1) ci
...
getCompRefIdList (IdList idvl il) n t = liftM2 combineCompInfo 
  (getCompRefIdVals idvl n t) (getCompRefIdentifiers il n t)
...
getCompRefIdVals ((IdVal id _):il) n t = liftM2 combineCompInfo 
  (getCompRefId id n t) (getCompRefIdVals il n t)
...
getCompRefIdentifiers ids n t = foldr (\id -> liftM2 combineCompInfo 
  (getCompRefId id n t)) (return []) ids
...
getCompRefId (Winner (CRef num)) n t = return $
    [(compnum, True, False) | not (t && num == 0)]
    where compnum = if num == 0 then n else num
getCompRefId (Loser (CRef num)) n t = return $
    [(compnum, False, True) | not (t && num == 0)]
    where compnum = if num == 0 then n else num
...
updatePhaseIds _ _ [] = []
updatePhaseIds pn an ((Act a):pl) = 
    Act (updateActionIds pn an a) : updatePhaseIds pn an pl
updatePhaseIds pn an ((Prog (AU au il)):pl) = 
    Prog (AU au (updateIdListIds pn an il)) : updatePhaseIds pn an pl
updatePhaseIds pn an ((Prog (CU cu il)):pl) = 
    Prog (CU cu (updateIdListIds pn an il)) : updatePhaseIds pn an pl
...
updateActionIds pn an (Comp (Scored c il)) = Comp 
  (Scored c (updateIdListIds pn an il))
updateActionIds pn an (Comp (Placed c il wn ln)) = Comp
  (Placed c (updateIdListIds pn an il) wn ln)
updateActionIds pn an (Dec (Vote il1 il2 sv)) = Dec (Vote 
  (updateIdListIds pn an il1) (updateIdListIds pn an il2) sv)
updateActionIds pn an (Dec (Nomination n il1 il2 sv)) = Dec (Nomination n 
  (updateIdListIds pn an il1) (updateIdListIds pn an il2) sv)
updateActionIds pn an (Dec (Allocation nm il)) = Dec 
  (Allocation nm (updateIdListIds pn an il))
updateActionIds pn an (Dec (DirectedVote il1 il2 sv)) = Dec (DirectedVote 
  (updateIdListIds pn an il1) (updateIdListIds pn an il2) sv)
updateActionIds pn an (Dec (Uses i ifpl elsepl)) = Dec (Uses (updateId pn an i) 
  (updatePhaseIds pn an ifpl) (updatePhaseIds pn an elsepl))
...
updateIdListIds pn an (IdList ivs is) = 
    IdList (updateIdValIds pn an ivs) (updateIdentifierIds pn an is)
...
updateIdValIds _ _ [] = []
updateIdValIds pn an ((IdVal id val):ivs) = 
    IdVal (updateId pn an id) val : updateIdValIds pn an ivs
...
updateIdentifierIds pn an = map (updateId pn an)
...
updateId pn an (N nm)
  ...
  | nm `elem` pn = N nm
  | otherwise = error $ 
    "Name " ++ nm ++ " does not match any player or affiliation"
...
updateTiebreakIds pn an (Tiebreak nm Nothing id) = Tiebreak nm Nothing 
  (updateId pn an id)
updateTiebreakIds pn an (Tiebreak nm (Just a) id) = Tiebreak nm 
  (Just (updateActionIds pn an a)) (updateId pn an id)
...
getAllAffiliations (G (PI pl tl _) rl _ tbs) pn = if any (isNameTaken pn) tl
    then error "Player and affiliation have same name. Unique names required."
    else nub $ "nominee" : (getAffsFromPlayerList pl pn ++ tl ++ concatMap 
      (`getAffsFromRound` pn) rl ++ concatMap (`getAffsFromTiebreaker` pn) tbs)
...
getAffsFromRound (R pl _ _) = getAffsFromPhaseList pl
...
getAffsFromPhaseList ((Prog (AU (Swap _ newnms False) _)):pl) pn = 
    if any (isNameTaken pn) newnms
    then error "Player and affiliation have same name. Unique names required."
