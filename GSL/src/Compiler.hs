...
compileTeams = foldr (($$) . compileTeam) empty
...
compilePlayers = foldr (($$) . compilePlayer) empty
...
compileAffiliations atl = brackets $ hcat $ intersperse (comma <> space) $ 
  map (doubleQuotes . text) (getAffsFromAttList atl [])
...
compileCountersFromAttList ((Affiliation _):atl) = 
  compileCountersFromAttList atl
compileCountersFromAttList (c:atl) = compileCounter c : 
  compileCountersFromAttList atl
...
compileRoundList rl = do
    ...
    return (first (text "roundList =" <+>) rldoc)
    where concatRounds :: [Round] -> Integer -> Reader IdNames (Doc, [Doc])
          ...
          concatRounds [R pl n ml] num = do
            ...
            return (brackets 
              (text "roundType" <> integer num) <+> text "*" <+> integer n, 
              [vcat 
                [text "def roundType" <> integer num <> parens empty <> colon, 
                nest 4 pldoc]])
...
compileCounterUpdate (Increase nm v) = do
    ...
    if nm `elem` counters ids
        then return $ text "for player in idList1: player.updateCounter" <> 
          parens (vdoc <> comma <+> doubleQuotes (text nm))
...
compileCounterUpdate (Decrease nm v) = do
    ...
    if nm `elem` counters ids
        then return $ text "for player in idList1: player.updateCounter" <> 
          parens (text "-" <+> parens vdoc <> comma <+> doubleQuotes (text nm))
        else error ("Reference to non-existent counter " ++ nm)
compileCounterUpdate (Set nm v) = do
    ids <- ask
    vdoc <- compileValue v
    if nm `elem` counters ids
        then return $ text "for player in idList1: player.setCounter" <> 
          parens (vdoc <> comma <+> doubleQuotes (text nm))
        else error ("Reference to non-existent counter " ++ nm)
...
compileAffUpdate (Add nm) = return $ 
  text "for player in idList1: player.addAff" <> parens (doubleQuotes (text nm))
...
compileAffUpdate (Change old new) = do
    ...
    if old `elem` affs ids
        then return $ text "for player in idList1: player.removeAff" <> 
          parens (doubleQuotes (text old)) <> semi <+> text "player.addAff" <> 
          parens (doubleQuotes (text new))
        else error ("Reference to non-existent affiliation " ++ old)
compileAffUpdate (Swap nms newnms np) = do
    ids <- ask
    if all (`elem` affs ids) nms
        then return $ text "game.swap" <> parens (compileNameList nms <> 
          comma <+> compileNameList newnms <> comma <+> text "idList1," <+> 
          text (show np))
        else error ("Reference to non-existent affiliation in " ++ show nms)
compileAffUpdate (Merge nms newnm) = do
    ids <- ask
    if all (`elem` affs ids) nms
        then return $ text "game.merge" <> parens (compileNameList nms <> 
          comma <+> mergeName newnm <> comma <+> text "idList1") 
        else error ("Reference to non-existent affiliation in " ++ show nms)
...
compileDec (Vote voterl voteel sv) = do
    ...
    return $ vcat [voterList, text "voterList = idList1", voteeList, 
      text "voteeList = idList1", text "game.vote" <> 
      parens (text "voterList, voteeList," <+> text (show sv))]
compileDec (Nomination n nommers pool sv) = do
    ...
    return $ vcat [nommerList, text "nommerList = idList1", poolList, 
      text "poolList = idList1", text "game.nominate" <> 
      parens (text "nommerList," <+> integer n <> comma <+> text "poolList," 
      <+> text (show sv))]
compileDec (Allocation nm il) = do
    ids <- ask
    ildoc <- compileIdentifierList il 1
    if nm `elem` counters ids
        then return $ vcat [ildoc, text "game.allocate" <> 
          parens (text "idList1," <+> doubleQuotes (text nm))]
        else error ("Reference to non-existent counter " ++ nm)
compileDec (DirectedVote voterl voteel sv) = do
    voterList <- compileIdentifierList voterl 1
    voteeList <- compileIdentifierList voteel 1
    return $ vcat [voterList, text "voterList = idList1", voteeList, 
      text "voteeList = idList1", text "game.directedVote" <> 
      parens (text "voterList, voteeList," <+> text (show sv))]
compileDec (Uses id yespl nopl) = do
    ...
    if null nopl
        then return $ vcat [iddoc, text "if uses" <> parens (text "ident") <> 
          colon, nest 4 yesdoc, nest 4 (delComps (countCompsInPhaseList yespl)),
          nest 4 (delVotes (countVotesInPhaseList yespl)), 
          nest 4 (delAllocs (countAllocsInPhaseList yespl))]
        else return $ vcat [iddoc, text "if uses" <> parens (text "ident") <> 
          colon, nest 4 yesdoc, nest 4 (delComps (countCompsInPhaseList yespl)),
          nest 4 (delVotes (countVotesInPhaseList yespl)), 
          nest 4 (delAllocs (countAllocsInPhaseList yespl)), text "else:", 
          nest 4 nodoc, nest 4 (delComps (countCompsInPhaseList nopl)), 
          nest 4 (delVotes (countVotesInPhaseList nopl)), 
          nest 4 (delAllocs (countAllocsInPhaseList nopl))]
...
compileComp (Scored Team il) = do
    ildoc <- compileIdentifierList il 1
    return $ vcat [ildoc, text "game.getScoredTeamCompResults" <> parens (text 
      "list" <> parens (text "dict.fromkeys" <> parens (brackets (text 
      "x for x in game.teamList for y in idList1 if x in y.affiliations"))))]
compileComp (Scored Individual il) = do
    ildoc <- compileIdentifierList il 1
    return $ vcat [ildoc, text "game.getScoredCompResults" <> 
      parens (text "idList1")]
compileComp (Placed Team il wn ln) = do
    ildoc <- compileIdentifierList il 1
    return $ vcat [ildoc, text "game.getTeamCompResults" <> parens (text 
      "list" <> parens (text "dict.fromkeys" <> parens (brackets (text 
      "x for x in game.teamList for y in idList1 if x in y.affiliations"))) <> 
      comma <+> text (show wn) <> comma <+> text (show ln))]
compileComp (Placed Individual il wn ln) = do
    ildoc <- compileIdentifierList il 1
    return $ vcat [ildoc, text "game.getCompResults" <> parens (text "idList1" 
      <> comma <+> text (show wn) <> comma <+> text (show ln))]
...
compileIdentifierList (IdList il el) n = do
    ...
    return $ vcat [inclList, exclList, text "idList" <> integer n <+> equals <+>
      brackets (text "x for x in includeList" <> integer n <+> 
      text "if x not in excludeList" <> integer n)]
    ...
    return $ vcat 
        ...
        where concatIdVals :: [IdentifierVal] -> Integer -> Reader IdNames Doc
              ...
              concatIdVals (iv:ivs) n = do
                ...
                return $ vcat [ivdoc, text "includeList" <> integer n <+> 
                  text "+= idVal", ivsdoc]
...
compileIdVal (IdVal id v) n = do
    ...
    return $ vcat [
        ...
        text "for player in ident" <> colon <+> text "idVal +=" <+> 
          brackets (text "player") <+> text "*" <+> vdoc]
...
compileIdentifiers il n = do
    ...
    return $ vcat [text "excludeList" <> integer n <+> equals <+> 
      brackets empty, exclList]
...
compileIdentifier (Chance num il) n = do
    ...
    return $ vcat [ildoc, text "ident =" <+> text "randomDraw" <> 
      parens (integer num <> comma <+> text "idList" <> integer (n+1))]
compileIdentifier Nominated _ = return $ text "ident =" <+> brackets (text 
  "x for x in game.playerList if" <+> doubleQuotes (text "nominee") <+>
  text "in x.affiliations")
...
compileIdentifier (N nm) _ = do
    ...
    if nm `elem` players ids
        then return $ text "ident =" <+> brackets (text 
          "x for x in game.playerList if x.name ==" <+> doubleQuotes (text nm))
        else error ("Reference to non-existent player" ++ nm) 
compileIdentifier (A af) _ = do
    ids <- ask 
    if af `elem` affs ids
        then return $ text "ident =" <+> brackets (text 
          "x for x in game.playerList if" <+> doubleQuotes (text af) <+> 
          text "in x.affiliations")
        else error ("Reference to non-existent affiliation" ++ af)
compileIdentifier (Winner cr) _ = return $ text "ident =" <+> brackets (text 
  "x for x in game.playerList if x ==" <+> compileCompRef cr <> brackets 
  (doubleQuotes (text "winner")) <+> text "or" <+> compileCompRef cr <> 
  brackets (doubleQuotes (text "winner")) <+> text "in x.affiliations")
compileIdentifier (Loser cr) _ = return $ text "ident =" <+> brackets (text 
  "x for x in game.playerList if x ==" <+> compileCompRef cr <> brackets 
  (doubleQuotes (text "loser")) <+> text "or" <+> compileCompRef cr <> brackets 
  (doubleQuotes (text "loser")) <+> text "in x.affiliations")
compileIdentifier (Majority vr Nothing) _ = return $ text "ident =" <+> 
  brackets (text "getVoteMinOrMax" <> parens (compileVoteRef vr <> comma <+> 
  text "True"))
compileIdentifier (Majority vr (Just tb)) n = do
    let tbdoc = compileTiebreakerRef tb
    return $ text "ident =" <+> brackets (text "getVoteMinOrMax" <> parens 
      (compileVoteRef vr <> comma <+> text "True" <> comma <+> tbdoc))
compileIdentifier (Minority vr Nothing) _ = return $ text "ident =" <+> 
  brackets (text "getVoteMinOrMax" <> parens (compileVoteRef vr <> comma <+> 
  text "False"))
compileIdentifier (Minority vr (Just tb)) n = do
    let tbdoc = compileTiebreakerRef tb
    return $ text "ident =" <+> brackets (text "getVoteMinOrMax" <> parens 
      (compileVoteRef vr <> comma <+> text "False" <> comma <+> tbdoc))
compileIdentifier (Most nm il Nothing) n = do
    ids <- ask
    ildoc <- compileIdentifierList il (n+1)
    if nm `elem` counters ids
        then return $ vcat [ildoc, text "ident =" <+> brackets (text 
          "getMinOrMax" <> parens (text "idList" <> integer (n+1) <> comma <+> 
          doubleQuotes (text nm) <> comma <+> text "True"))]
        else error ("Reference to non-existent counter " ++ nm)
...
compileTiebreaker (Tiebreak (firstletter:nm) (Just a) id) = do
    ...
    return $ vcat [text "def" <+> text (toLower firstletter:nm) <> text 
      ...
      nest 4 (delResultIfNeeded a), nest 4 (text "return ident[0]")]
    where delResultIfNeeded (Comp _) = text "del game.compResults[-1]"
          delResultIfNeeded (Dec Vote {}) = text "del game.voteResults[-1]"
          delResultIfNeeded (Dec DirectedVote {}) = text 
...
compileTiebreakerRef (TieRef (firstletter:n)) = text (toLower firstletter:n) <> 
  text "Tiebreaker"
...
compileValue (Result (Cmp cr)) = return $ compileCompRef cr <> brackets 
  (doubleQuotes (text "scores")) <> brackets (text "player") <+> text 
  "if player in" <+> compileCompRef cr <> brackets (doubleQuotes 
  (text "scores")) <> text ".keys() else" <+> compileCompRef cr <> brackets 
  (doubleQuotes (text "scores")) <> brackets (brackets (text 
  "x for x in game.teamList if x in player.affiliations") <> 
  brackets (text "0")) 
compileValue (Result (Vt vr)) = return $ compileVoteRef vr <> brackets 
  (text "player")
compileValue (Result (Alloc ar)) = return $ compileAllocRef ar <> brackets 
  (text "player")
...
compileWinCondition Survive = return $ vcat [
...
compileWinCondition (Jury num) = return $ vcat [
    text "game.juryVote" <> parens (integer num), 
    text "winner = getVoteMinOrMax(game.voteResults[-1], True)", 
    text "print" <> parens (text "winner.name +" <+> doubleQuotes (text " won!"))]
compileWinCondition (FinalComp Team) = return $ vcat [
    text "game.getTeamCompResults(game.teamList, True, False)",
    text "winner = game.compResults[-1]" <> brackets (doubleQuotes (text "winner")), 
    text "print" <> parens (text "winner +" <+> doubleQuotes (text " won!"))]
compileWinCondition (FinalComp Individual) = return $ vcat [
    text "game.getCompResults(game.playerList, True, False)", 
    text "winner = game.compResults[-1]" <> brackets (doubleQuotes (text "winner")), 
    text "print" <> parens (text "winner.name +" <+> doubleQuotes (text " won!"))]
compileWinCondition (Reach gl Individual) = do
    ...
    return $ vcat [
        nest 4 (text "winners = []"), 
        nest 4 (text "for player in game.playerList:"), 
        nest 8 (text "if" <+> gldoc <> colon), 
        nest 12 (text "winners.append(player.name)"), 
        nest 4 (text "if len(winners) > 0:"), 
        nest 8 (text "winnerString =" <+> doubleQuotes empty), 
        nest 8 (text "if len(winners) == 1:"), 
        nest 12 (text "winnerString = winners[0]"), 
        nest 8 (text "else:"), 
        nest 12 (text "for winner in winners[1:]:"), 
        nest 16 (text "winnerString += winner +" <+> doubleQuotes (comma <> space)), 
        nest 12 (text "winnerString +=" <+> doubleQuotes (text "and ") <+> text "+ winners[0]"), 
        nest 8 (text "print" <> parens (text "winnerString +" <+> doubleQuotes (text " won!"))), 
        nest 8 (text "break")]
compileWinCondition (Reach gl Team) = do
    ...
    return $ vcat [
        ...
        nest 4 (text "for player in game.playerList:"), 
        nest 8 (text "if" <+> gldoc <> colon), 
        nest 12 (text "winners.append(player)"), 
        nest 4 (text "if len(winners) > 0:"), 
        nest 8 (text "winTeams = list" <> parens (text "dict.fromkeys" <> 
          parens (brackets (text "x for x in game.teamList for y in winners if x in y.affiliations")))), 
        nest 8 (text "winnerString =" <+> doubleQuotes empty), 
        nest 8 (text "if len(winTeams) == 1:"), 
        nest 12 (text "winnerString = winTeams[0]"), 
        nest 8 (text "else:"), 
        nest 12 (text "for winner in winTeams[1:]:"), 
        nest 16 (text "winnerString += winner +" <+> doubleQuotes (comma <> space)), 
        nest 12 (text "winnerString +=" <+> doubleQuotes (text "and ") <+> text "+ winTeams[0]"), 
        nest 8 (text "print" <> parens (text "winnerString +" <+> doubleQuotes (text " won!"))), 
        nest 8 (text "break")]
...
compileGoalList [Gl num nm] = do
    ids <- ask
    if nm `elem` counters ids
        then return $ text "player.checkWinCondition" <> parens (doubleQuotes 
          (text nm) <> comma <+> integer num)
        else error ("Reference to non-existent counter " ++ nm)
compileGoalList ((Gl num nm):gl) = do
    ...
    if nm `elem` counters ids
        then return $ text "player.checkWinCondition" <> parens (doubleQuotes 
          (text nm) <> comma <+> integer num) <+> text "and" <+> gldoc
        else error ("Reference to non-existent counter " ++ nm)
...
countVotesInPhaseList ((Act (Dec Vote {})):pl) = 1 + countVotesInPhaseList pl
countVotesInPhaseList ((Act (Dec DirectedVote {})):pl) = 
    1 + countVotesInPhaseList pl
...
countAllocsInPhaseList ((Act (Dec (Allocation _ _))):pl) =
    1 + countAllocsInPhaseList pl
...
getCountersFromPlayerList ((P _ al):pl) = getCountersFromAttList al ++ 
  getCountersFromPlayerList pl
