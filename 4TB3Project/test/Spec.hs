    ...
    describe "Compiler" $ do
        ...
        describe "compileDec" $ do
            ...
            it "compiles a DirectedVote into a call to a python function to get the vote results" $
                runReader (compileDec (DirectedVote (IdList [IdVal Everyone (Num 1)] [N "Brooks"]) (IdList [IdVal Everyone (Num 1)] [A "Test"]) False)) ids `shouldBe` (text "includeList1 = []\nident = game.playerList\nidVal = ident\nincludeList1 += idVal\nexcludeList1 = []\nident = [x for x in game.playerList if x.name == \"Brooks\"]\nexcludeList1 += ident\nidList1 = [x for x in includeList1 if x not in excludeList1]\nvoterList = idList1\nincludeList1 = []\nident = game.playerList\nidVal = ident\nincludeList1 += idVal\nexcludeList1 = []\nident = [x for x in game.playerList if \"Test\" in x.affiliations]\nexcludeList1 += ident\nidList1 = [x for x in includeList1 if x not in excludeList1]\nvoteeList = idList1\ndirectedVote(voterList, voteeList, False)", [])
            -- it "compiles a Uses into a call to a python function to get the decision results and a conditional based on the result" $
            --     runReader (compileDec (Uses (N "Brooks") [Act (Comp (Scored Individual (IdList [] [Everyone])))] [Act (Comp (Scored Individual (IdList [IdVal Everyone (Num 1)] [])))])) ids `shouldBe` (text "ident = [x for x in game.playerList if x.name == \"Brooks\"]\nif uses(ident):\n    includeList1 = []\n    excludeList1 = []\n    ident = game.playerList\n    excludeList1 += ident\n    idList1 = [x for x in includeList1 if x not in excludeList1]\n    game.getScoredCompResults(idList1)\nelse:\n    includeList1 = []\n    ident = game.playerList\n    idVal = ident\n    includeList1 += idVal\n    excludeList = []\nidList1 = [x for x in includeList1 if x not in excludeList1]\n    game.getScoredCompResults(idList1)", [])
